...
from model.encoder.fa import FrameAveraging
from model.encoder.nn_utils import MLPWrapper
...
class FAFFN(FrameAveraging):
    ...
    def forward(self, token_embs, geo_feats, batch_idx):     
        ...
        batch_geo_feats, coord_mask = to_dense_batch(geo_feats, batch=batch_idx)

        B, N = batch_token_embs.size(0), batch_token_embs.size(1)
        
        geo_frame_feats, F_ops, center = self.create_frame(batch_geo_feats, coord_mask)
        geo_frame_feats = self.W_frame(geo_frame_feats).view(B, 8, N, -1).mean(dim=1)

        h_token_embs = torch.cat([batch_token_embs, geo_frame_feats], dim=-1)
...
class EdgeModule(FrameAveraging):
    ...
    def forward(self, token_embs, geo_feats, neighbor_indices, neighbor_masks):
        ...
        radial_coords = geo_feats.unsqueeze(dim=1) - geo_feats[neighbor_indices]
        radial_coord_norm = torch.sum(radial_coords ** 2, dim=-1).unsqueeze(-1)
        
        """local frame features"""
        frame_feats, _, _ = self.create_frame(radial_coords, neighbor_masks)
        frame_feats = frame_feats.view(N, 8, N_neighbors, -1)

        radial_coord_norm = radial_coord_norm.unsqueeze(dim=1).expand(N, 8, N_neighbors, -1)
        frame_feats = self.coord_mlp(torch.cat([frame_feats, radial_coord_norm], dim=-1)).mean(dim=1)

        pair_embs = torch.cat([token_embs.unsqueeze(dim=1).expand(N, N_neighbors, -1), token_embs[neighbor_indices]], dim=-1)
...
class MLPAttnEdgeAggregation(FrameAveraging):
    ...
    def forward(self, token_embs, geo_feats, edge_feats, neighbor_indices, batch_idx, neighbor_masks):
        ...
        gate_s = self.W_gate(token_embs).sigmoid()
        ...
        attn_map = attn_map + self.edge_attn(q_edge_s).squeeze(dim=-1)
        ...
        edge_context = einsum(attn_map, v_edge_s, 'n h m, n m h d -> n h d').view(n_tokens, -1)
        ...
        if n_heads == 1:
            ...
            geo_frame_feats = geo_frame_feats.view(batch_B, 8, batch_N, 3).transpose(2, 1).reshape(batch_B, batch_N, 8*3)
            geo_frame_feats_flattern = geo_frame_feats[coord_mask].view(n_tokens, 8, 3).transpose(1, 0)
            ...
            neighbor_indices_expand = neighbor_indices.unsqueeze(dim=0).expand(8, -1, -1).reshape(8*n_tokens, -1)

            geo_neighbors = geo_frame_feats_flattern[neighbor_indices_expand].view(8, n_tokens, -1, n_heads, 3)
            attn_map_expand = attn_map.unsqueeze(dim=0).expand(8, -1, -1, -1).transpose(3, 2)

            geo_context = einsum(attn_map_expand, geo_neighbors, 'f n m h, f n m h d -> f n h d')
            geo_context = self.W_frame_agg(geo_context.transpose(3, 2)).squeeze(-1).transpose(1, 0).reshape(n_tokens, -1)

            batch_geo_context, _ = to_dense_batch(geo_context, batch=batch_idx)
            batch_geo_context = batch_geo_context.view(batch_B, batch_N, 8, 3).transpose(2, 1)
            geo_context = self.invert_frame(batch_geo_context, coord_mask, F_ops, center)
            geo_context = geo_context[coord_mask]
...
class FAFormerEncoderLayer(nn.Module):
    ...
    def forward(self, token_embs, geo_feats, edge_feats, neighbor_indices, batch_idx, neighbor_masks):
        ...
        token_embs, geo_feats = self.self_attn(token_embs, geo_feats, edge_feats, neighbor_indices, batch_idx, neighbor_masks)#第一块
...
class FAFormer(nn.Module):
    ...
    def forward(self, features, coords, pad_mask=None):
        ...
        token_embs = self.input_transform(features)

        pos_tokens = torch.arange(N).unsqueeze(0).repeat(B, 1).to(device)
        token_embs = token_embs + self.pos_emb(pos_tokens[~pad_mask])
